...
export class AlertError extends Error {
  ...
  get name(): string {
    ...
    return typeof name !== 'undefined' && name !== '' ? name : this._name;
  }
...
export const ErrBadCode = new AlertError({
  message:
    '<p>If you’re sure this is the right code: Either the sender is no longer connected, or the code was already used.</p>' +
...
export const ErrRecvConnectionTimeout = new AlertError({
  ...
  message:
    '<p>It looks like the connection between you and the sender was briefly lost.</p>' +
...
export const ErrRelay = new AlertError({
  message: ServerErrorMsg,
  pattern:
    '(^websocket.Dial failed|failed to establish connection$)|(.*$transitRelayURL.*)',
});
...
export const ErrInterrupt = new AlertError({
  ...
  pattern:
    '(^failed to read: WebSocket closed: unclean connection.*status = StatusAbnormalClosure.*reason = ""$)|(.*$transitRelayURL.*)',
});

export const ErrInvalidCode = new Error(
