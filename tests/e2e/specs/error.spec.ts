...
async function initGo() {
  ...
  await WebAssembly.instantiateStreaming(
    fetch(`${TEST_HOST}/assets/wormhole.wasm`),
    go.importObject
  ).then((result) => {
...
describe('Error messaging', () => {
  ...
  describe('Receiving', () => {
    it('should alert mailbox server is unavailable', () => {
      ...
      cy.fixture(filename).then(async (file: string) => {
        ...
        cy.window().then((window) => {
          ...
          cy.get('input[type="text"]').type('12-not-real');

          cy.get('.receive-next').click();
    ...
    it('should alert relay server is unavailable', () => {
      ...
      cy.fixture(filename).then(async (file: string) => {
        ...
        cy.window().then((window) => {
          ...
          cy.get('input[type="text"]').type(code as string);

          cy.get('.receive-next').click().wait(500);
    ...
    it.skip('should show a specific error when the transfer is interrupted on the send-side', async (testDone) => {
      ...
      const expected = await largeUint8ArrToString(
        new Uint8Array(file.arrayBuffer())
      );
      const { code, cancel, done } = await sender.sendFile(
