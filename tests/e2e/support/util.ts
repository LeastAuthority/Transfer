...
export function expectFileDownloaded(
  filename: string,
  expected: string
): Chainable<undefined> {
  ...
  return cy
    ...
    .then(() => {
      ...
      return cy.readFile(path, 'utf-8', { timeout: 3000 }).should((actual) => {
...
export async function mockClientSend(
  name: string,
  data: string,
...
export async function mockGetReceiveReader(
  code: string
): Promise<FileStreamReader> {
  const go = new Go();
  await WebAssembly.instantiateStreaming(
    fetch('http://localhost:8080/assets/wormhole.wasm'),
    go.importObject
  ).then((result) => {
